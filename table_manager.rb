...
### HTML - Insert code for Email. 
  ...
  # Insert links - with NORMAL version
  def insert_links_normal(output_file)
  ...
  ## With this code, the links in CSV must be ordered (same as photoshop slices)
    ...
    # doc.xpath("//a").each_with_index do |a, i|
    #   # Banner & CTA
    #   next if i > @content.length - 1 or @content[i][:cta_link].nil? or !@content[i][:cta_link].include? 'http' or !a.xpath("//a//img//@alt") == @content[i][:cta]
    #     a['href'] = @content[i][:cta_link]
    # end
    File.open(output_file, "w") {|out| out << doc.to_s }
  end 
